package cn.lucode.fastdev.util.httpclient;
...
public class HttpClientUtil{
    ...
    static{
        try {
            ...
            ParamUtils.errorException("创建https协议的HttpClient对象出错：{}", e);
        }
    }
    ...
    private static HttpResponse execute(HttpConfig config) throws HttpProcessException {
        ...
        try {
            ...
            if(HttpEntityEnclosingRequestBase.class.isAssignableFrom(request.getClass())){
                ...
                //检测url中是否存在参数
                config.url(ParamUtils.checkHasParas(config.url(), nvps, config.inenc()));

                //装填参数
                HttpEntity entity = ParamUtils.map2HttpEntity(nvps, config.map(), config.inenc());

                //设置参数到请求对象中
                ((HttpEntityEnclosingRequestBase)request).setEntity(entity);

                ParamUtils.info("请求地址："+config.url());
                if(nvps.size()>0){
                    ParamUtils.info("请求参数："+nvps.toString());
                }
                if(config.json()!=null){
                    ParamUtils.info("请求参数："+config.json());
                }
            }else{
                int idx = config.url().indexOf("?");
                ParamUtils.info("请求地址："+config.url().substring(0, (idx>0 ? idx : config.url().length())));
                if(idx>0){
                    ParamUtils.info("请求参数："+config.url().substring(idx+1));
                }
            }
            //执行请求操作，并拿到结果（同步阻塞）
